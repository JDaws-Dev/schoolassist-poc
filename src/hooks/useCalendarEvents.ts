...
export function useCalendarEvents(calendarUrl: string | null = DEFAULT_CALENDAR_URL) {
  ...
  const loadFromCache = useCallback(() => {
    try {
      ...
      if (cached) {
        const { events: cachedEvents, timestamp, url } = JSON.parse(cached) as {
    ...
    async (useCache = true) => {
      ...
      try {
        ...
        if (!calendarUrl) {
          ...
          // Use the API proxy to avoid CORS issues with Google Calendar
  ...
  const refresh = useCallback(() => fetchEvents(false), [fetchEvents])
